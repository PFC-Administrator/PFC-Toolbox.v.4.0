...
namespace PFC_Toolbox.v._4._0.Controllers
{
    public class SMSController : Controller
    {
                    ...
                    using (SqlCommand cmd = new SqlCommand("[Toolbox-ItemLookupByDescription]", con))
                    {
                        ...
                        if (reader.HasRows)
                        {
                            while (reader.Read())
                            {
                                ...
                                // Store results in ItemLookupByDescriptionModel model
                    ...
                    using (SqlCommand cmd = new SqlCommand("[Toolbox-ItemLookupByDescription]", con))
                    {
                        ...
                        if (reader.HasRows)
                        {
                            while (reader.Read())
                            {
                                ...
                                // Store results in ItemLookupByDescriptionyModel model
                                item = new ItemLookupByDescriptionModel();
                                item.ItemCode = reader["ItemCode"].ToString();
                                item.ItemBrand = reader["Brand"].ToString();
                                item.ItemDescription = reader["ReceiptDescription"].ToString();
        ...
        // GET: /SMS/ItemLookupByBrand
        public ActionResult ItemLookupByBrand()
        {
            return View();
        }

        public ActionResult GetItemCodeInfoByBrand(string brand, string location)
        {
            // Creates variables for use within stored procedure and report display
            string storeCode = location.Split(',')[0];
            string storeDescription = location.Split(',')[1];
            ...
            // Create local objects
            SqlDataReader reader = null;
            List<ItemLookupByBrandModel> report = new List<ItemLookupByBrandModel>();
            ItemLookupByBrandModel item = null;

            if (location.Equals("001, La Crosse"))
            {
                // Connect to Host SMS and run Toolbox-ItemLookupByBrand stored procedure
                using (SqlConnection con = new SqlConnection { ConnectionString = ConfigurationManager.ConnectionStrings["SMSLaxConnection"].ConnectionString })
                {
                    using (SqlCommand cmd = new SqlCommand("[Toolbox-ItemLookupByBrand]", con))
                    {
                        // Set the command type as a stored procedure
                        cmd.CommandType = CommandType.StoredProcedure;
                        ...
                        if (reader.HasRows)
                        {
                            while (reader.Read())
                            {
                                ...
                                // Store results in ItemLookupByBrandModel model
                                item = new ItemLookupByBrandModel();
                                item.ItemCode = reader["ItemCode"].ToString();
                                item.ItemBrand = reader["Brand"].ToString();
                                item.ItemDescription = reader["ReceiptDescription"].ToString();
                                item.ItemLongDesc = reader["LongDescription"].ToString();
                ...
                return PartialView("_ItemLookupByBrand", report);
            }
            else if (location.Equals("002, Rochester"))
            {
                // Connect to Host SMS and run Toolbox-ItemLookupByBrand stored procedure
